...
class ContentGenerator {
     ...
     * 🆕 Version améliorée avec parsing GRANULAIRE
     */
    parsePageContent(rawContent, pageType) {
        ...
        if (pageType === 'home') {
            ...
            // 🆕 PARSING GRANULAIRE: Propositions de valeur / Features
            const propositions = this.extractStructuredSection(rawContent, 'PROPOSITIONS DE VALEUR');
            if (propositions) {
                content.features = this.parseGranularFeatures(propositions);
            ...
            // 🆕 PARSING GRANULAIRE: Services
            const services = this.extractStructuredSection(rawContent, 'NOS SERVICES');
            if (services) {
                content.services = this.parseGranularServices(services);
            ...
            // 🆕 PARSING GRANULAIRE: Pricing / Tarifs
            const pricing = this.extractStructuredSection(rawContent, 'TARIFS');
            if (pricing) {
                content.pricing = this.parseGranularPricing(pricing);
            ...
            // 🆕 PARSING GRANULAIRE: Témoignages
            const testimonials = this.extractStructuredSection(rawContent, 'TÉMOIGNAGES');
            if (testimonials) {
                content.testimonials = this.parseGranularTestimonials(testimonials);
            ...
            if (contact) {
                ...
                content.contact_info = this.parseGranularContact(contact);
            }

            // 🆕 PARSING GRANULAIRE: Pourquoi nous choisir / Benefits
            const pourquoi = this.extractStructuredSection(rawContent, 'POURQUOI NOUS CHOISIR');
            if (pourquoi) {
                content.benefits = this.parseGranularBenefits(pourquoi);
    ...
    // ═══════════════════════════════════════════════════════════════════════
    // 🆕 MÉTHODES DE PARSING GRANULAIRE
    // ═══════════════════════════════════════════════════════════════════════

    /**
     * Parse les features/propositions de valeur en objets individuels
     */
    parseGranularFeatures(text) {
        const features = [];

        // Format 1: ▸ TITRE: ... DESCRIPTION: ...
        if (text.includes('TITRE:') || text.includes('▸')) {
            const items = text.split(/▸/).filter(item => item.trim());
            items.forEach((item, index) => {
                const titleMatch = item.match(/TITRE:\s*([^\n]+)/i);
                const descMatch = item.match(/DESCRIPTION:\s*([\s\S]*?)(?=▸|$)/i);
        ...
        // Format 2: Lignes avec • ou -
        if (features.length === 0) {
            const lines = text.split('\n').filter(l => l.trim());
            lines.forEach((line, index) => {
                const cleaned = line.replace(/^[•\-*▸]\s*/, '').trim();
                if (cleaned && index < 6) {
                    // Séparer titre et description si possible
                    const parts = cleaned.split(/[:\-–]/);
                    features.push({
                        title: parts[0]?.trim() || cleaned.substring(0, 50),
    ...
    /**
     * Parse les témoignages en objets individuels
     */
    parseGranularTestimonials(text) {
        const testimonials = [];

        // Format: "Citation" - Auteur, Position
        const quotes = text.split(/\n\n/).filter(t => t.trim());

        quotes.forEach((quote, index) => {
            if (index >= 3) return; // Max 3 témoignages

            const quoteMatch = quote.match(/"([^"]+)"/);
            const authorMatch = quote.match(/[-–]\s*([^,\n]+)/);
            const positionMatch = quote.match(/[-–]\s*[^,]+,\s*([^\n]+)/);

            if (quoteMatch) {
                testimonials.push({
                    text: quoteMatch[1].trim(),
                    author: authorMatch ? authorMatch[1].trim() : 'Client satisfait',
    ...
    /**
     * Parse les statistiques/preuves sociales
     */
    parseGranularStats(text) {
        const stats = [];

        // Chercher des patterns comme "100+", "5 ans", "98%", etc.
        const patterns = [
            /(\d+\+?)\s*(clients?|projets?|années?|ans)/gi,
            /(\d+%)\s*(satisfaction|réussite|fidélité)/gi,
            /(24\/7|7j\/7)\s*(support|disponible|assistance)/gi,
            /(\d+)\s*(experts?|professionnels?)/gi
        ];
        ...
        // Si pas de stats trouvées, extraire des lignes
        if (stats.length === 0) {
            const lines = text.split('\n').filter(l => l.trim());
            lines.slice(0, 4).forEach(line => {
                ...
                if (numMatch) {
                    stats.push({
                        number: numMatch[1],
                        label: line.replace(numMatch[0], '').trim().substring(0, 30)
                    });
                }
            });
        }
    ...
    /**
     * Parse les services en objets individuels
     */
    parseGranularServices(text) {
        const services = [];

        // Format: SERVICE X: Nom \n Description
        const serviceMatches = text.matchAll(/SERVICE\s*\d*:\s*([^\n]+)\n([\s\S]*?)(?=SERVICE\s*\d*:|$)/gi);

        for (const match of serviceMatches) {
            if (services.length < 6) {
                services.push({
                    name: match[1].trim(),
                    description: match[2].trim().substring(0, 300)
                });
            }
        }

        // Fallback: format avec ### ou ##
    ...
    /**
     * Parse les tarifs en objets individuels
     */
    parseGranularPricing(text) {
        const pricing = [];

        // Format: OFFRE X: Nom \n Prix: XXX€ \n Description
        const offerMatches = text.matchAll(/OFFRE\s*\d*:\s*([^\n]+)\n.*?Prix:\s*([^\n]+)\n([\s\S]*?)(?=OFFRE\s*\d*:|$)/gi);
        ...
        for (const match of offerMatches) {
            if (pricing.length < 3) {
                pricing.push({
                    ...
                    popular: match[1].toLowerCase().includes('populaire') || match[1].includes('⭐')
                });
            }
        }
    ...
    /**
     * Parse les bénéfices/avantages en objets individuels
     */
    parseGranularBenefits(text) {
        const benefits = [];

        const lines = text.split('\n').filter(l => l.trim());
        lines.forEach((line, index) => {
            if (index >= 5) return; // Max 5 bénéfices
    ...
    /**
     * Parse les informations de contact
     */
    parseGranularContact(text) {
        const contact = {};
        ...
        // Téléphone
        const phoneMatch = text.match(/(\+33|0)[\s.-]?\d([\s.-]?\d{2}){4}/);
        if (phoneMatch) contact.phone = phoneMatch[0];

        // Adresse
        const addressMatch = text.match(/\d+[,\s]+[\w\s]+,?\s*\d{5}\s*[\w-]+/i);
        if (addressMatch) contact.address = addressMatch[0];

        // Horaires
        const hoursMatch = text.match(/(\d{1,2}h?\s*[-–àa]\s*\d{1,2}h?)/i);
        if (hoursMatch) contact.hours = hoursMatch[0];
