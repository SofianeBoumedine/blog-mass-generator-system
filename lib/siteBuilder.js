...
class SiteBuilder {
    constructor() {
        ...
        this.thematicFallbacks = null; // 🆕 Fallbacks thématiques
    }
    ...
    async initialize() {
        ...
        // 🆕 Charger les fallbacks thématiques
        const fallbacksPath = path.join(__dirname, '..', 'config', 'thematic-fallbacks.json');
        try {
            this.thematicFallbacks = JSON.parse(await fs.readFile(fallbacksPath, 'utf8'));
            console.log('📦 Fallbacks thématiques chargés');
        } catch (e) {
            console.warn('⚠️ Fichier thematic-fallbacks.json non trouvé, utilisation des fallbacks par défaut');
            this.thematicFallbacks = {};
        }
    ...
    buildPage(pageType, content, branding, framework, layoutTemplate, colorScheme, navigation) {
        ...
        // 🆕 Détecter le thème pour les fallbacks thématiques
        const detectedTheme = content.metadata?.theme || content.theme || 'general';

        // 🆕 NOUVELLES VARIABLES DE FONCTIONNALITÉS (avec thème)
        html = this.replaceFeatureVariables(html, content, detectedTheme);

        // 🆕 NOUVELLES VARIABLES DE TÉMOIGNAGES (avec thème)
        html = this.replaceTestimonialVariables(html, content, detectedTheme);

        // 🆕 NOUVELLES VARIABLES DE STATISTIQUES (avec thème)
        html = this.replaceStatsVariables(html, content, detectedTheme);

        // 🆕 CTA thématique
        const thematicCTA = this.getThematicCTA(detectedTheme);
        html = html.replace(/{cta_thematic}/g, thematicCTA);
...
`;
     ...
     * Version améliorée avec parsing granulaire et fallbacks thématiques
     */
    replaceFeatureVariables(html, content, theme = 'general') {
        // 🆕 Récupérer les features depuis le contenu parsé OU les fallbacks thématiques
        const parsedFeatures = content.features || [];
        const thematicFeatures = this.getThematicFallback(theme, 'features') || [];

        // Fusionner: priorité au contenu parsé, puis fallbacks thématiques
        const features = [];
        for (let i = 0; i < 6; i++) {
            if (parsedFeatures[i]) {
                features.push(parsedFeatures[i]);
            } else if (thematicFeatures[i]) {
                features.push(thematicFeatures[i]);
            } else {
                features.push({ title: '', description: '' });
            }
        }

        // Remplacer les variables individuelles
        for (let i = 0; i < 6; i++) {
            const feature = features[i];
            html = html.replace(new RegExp(`{feature_${i+1}_title}`, 'g'), feature.title || '');
            html = html.replace(new RegExp(`{feature_${i+1}_description}`, 'g'), feature.description || '');
        }
     ...
     * Version améliorée avec parsing granulaire et fallbacks thématiques
     */
    replaceTestimonialVariables(html, content, theme = 'general') {
        ...
        // 🆕 Récupérer les témoignages depuis le contenu parsé OU les fallbacks thématiques
        const parsedTestimonials = content.testimonials || [];
        const thematicTestimonials = this.getThematicFallback(theme, 'testimonials') || [];

        // Fusionner: priorité au contenu parsé, puis fallbacks thématiques
        const testimonials = [];
        for (let i = 0; i < 3; i++) {
            if (parsedTestimonials[i]) {
                testimonials.push(parsedTestimonials[i]);
            } else if (thematicTestimonials[i]) {
                testimonials.push(thematicTestimonials[i]);
            } else {
        ...
        for (let i = 0; i < 3; i++) {
            ...
            const textWithQuotes = t.text ? `"${t.text.replace(/^"|"$/g, '')}"` : '';
            html = html.replace(new RegExp(`{testimonial_${i+1}_text}`, 'g'), textWithQuotes);
            html = html.replace(new RegExp(`{testimonial_${i+1}_author}`, 'g'), t.author || '');
            html = html.replace(new RegExp(`{testimonial_${i+1}_position}`, 'g'), t.position || '');
        }
     ...
     * Version améliorée avec parsing granulaire et fallbacks thématiques
     */
    replaceStatsVariables(html, content, theme = 'general') {
        // 🆕 Récupérer les stats depuis le contenu parsé OU les fallbacks thématiques
        const parsedStats = content.stats || [];
        const thematicStats = this.getThematicFallback(theme, 'stats') || [];

        // Fusionner: priorité au contenu parsé, puis fallbacks thématiques
        const stats = [];
        for (let i = 0; i < 4; i++) {
            if (parsedStats[i]) {
                stats.push(parsedStats[i]);
            } else if (thematicStats[i]) {
                stats.push(thematicStats[i]);
            } else {
        ...
        for (let i = 0; i < 4; i++) {
            const stat = stats[i];
            html = html.replace(new RegExp(`{stat_${i+1}_number}`, 'g'), stat.number || '');
            html = html.replace(new RegExp(`{stat_${i+1}_label}`, 'g'), stat.label || '');
        }

        // Variable legacy pour le support
        html = html.replace(/{stat_label_support}/g, stats[3]?.label || 'Support Disponible');
    ...
    // ═══════════════════════════════════════════════════════════════════════
    // 🆕 MÉTHODES POUR FALLBACKS THÉMATIQUES
    // ═══════════════════════════════════════════════════════════════════════

    /**
     * 🆕 Récupère un fallback thématique pour une catégorie donnée
     * @param {string} theme - Le thème détecté (restaurant, medical, fitness, etc.)
     * @param {string} category - La catégorie (features, testimonials, stats, benefits, cta)
     * @returns {Array|string|null} - Les données du fallback ou null
     */
    getThematicFallback(theme, category) {
        if (!this.thematicFallbacks) return null;

        // Mapping des thèmes vers les clés du fichier JSON
        const themeMapping = {
        ...
        const normalizedTheme = theme?.toLowerCase().replace(/[^a-z]/g, '') || 'general';
        const themeKey = themeMapping[normalizedTheme] || 'general';

        // Récupérer les données du thème
        const themeData = this.thematicFallbacks[themeKey] || this.thematicFallbacks['general'];

        if (!themeData) return null;

        return themeData[category] || null;
    }

    /**
     * 🆕 Récupère le CTA thématique approprié
     * @param {string} theme - Le thème détecté
     * @returns {string} - Le texte du CTA
     */
    getThematicCTA(theme) {
        const cta = this.getThematicFallback(theme, 'cta');
        return cta || 'Nous contacter';
    }

    /**
     * 🆕 Détermine le thème à partir de l'analyse
     * @param {Object} analysis - L'analyse du contenu
     * @param {Object} content - Le contenu généré
     * @returns {string} - Le thème identifié
     */
    detectThemeFromAnalysis(analysis, content) {
        // Priorité: theme explicite > sector > businessType > 'general'
        if (analysis?.theme) return analysis.theme;
        if (analysis?.sector) return analysis.sector;
        if (analysis?.businessType) return analysis.businessType;
        if (content?.metadata?.theme) return content.metadata.theme;
        return 'general';
    }
}
