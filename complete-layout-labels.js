/**
 * Script pour compléter les labels manquants des layouts 21-50
 */

const fs = require('fs');
const path = require('path');

// Labels manquants pour les layouts 21-50
const missingLayoutLabels = [
    {
...
console.log('\n📊 Labels des layouts 21-50 (30 layouts):');
console.log('═══════════════════════════════════════════════════════════════════\n');

missingLayoutLabels.forEach(layout => {
    console.log(`✅ ${layout.layout}`);
    console.log(`   Type: ${layout.type}`);
    console.log(`   Quality: ${layout.quality}`);
    console.log(`   Themes: ${layout.themes.join(', ')}`);
    console.log(`   Mood: ${layout.mood}`);
    console.log(`   Universal: ${layout.universal}`);
    console.log('');
});

console.log('═══════════════════════════════════════════════════════════════════');
console.log(`\n🎯 Total: ${missingLayoutLabels.length} layouts labellisés`);
console.log('\n💡 Pour intégrer ces labels dans intelligentLayoutSelectorV2.js,');
console.log('   ajoutez ces objets dans le tableau layoutDatabase entre les');
console.log('   sections "LAYOUTS VISUELS/EFFETS" et "LAYOUTS SECTORIELS 51-70"\n');

// Générer le code JavaScript formaté
const jsCode = missingLayoutLabels.map(l => {
    return `            {
                layout: '${l.layout}',
                type: '${l.type}',
                quality: '${l.quality}',
...
fs.writeFileSync('layouts-21-50-labels.txt', jsCode);
console.log('📝 Code JavaScript sauvegardé dans: layouts-21-50-labels.txt\n');
